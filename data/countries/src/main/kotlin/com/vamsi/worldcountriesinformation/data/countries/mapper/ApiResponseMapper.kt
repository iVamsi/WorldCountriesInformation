...
/**
 * Mapper functions to convert API response models to domain models
 */

fun List<CountriesResponseItem>.toCountries(): List<Country> {
    return this.map { countriesResponseItem ->
        val languages = countriesResponseItem.languages?.map {
            Language(it.name, it.nativeName)
        }?.toList()

        val currencies = countriesResponseItem.currencies?.map {
